...
- [ ] 新建 `scripts/modules/module_debuff_aura.gd`，实现：
...
- [ ] `@export` 参数：`aura_radius: float = 5.0`、`debuff_speed_mult: float = 0.7`、`debuff_damage_mult: float = 0.8`

### SpeedAura 模块（友方加速）

- [ ] 新建 `scripts/modules/module_speed_aura.gd`，实现：
...
- [ ] `@export` 参数：`aura_radius: float = 10.0`、`speed_mult: float = 1.2`

### HealAura 模块（友方回血）

- [ ] 新建 `scripts/modules/module_heal_aura.gd`，实现：
...
- [ ] `@export` 参数：`aura_radius: float = 8.0`、`heal_per_second: float = 3.0`

### 测试

- [ ] `tests/test_module_debuff_aura.gd`：创建怪物挂 DebuffAura → 玩家进入 5m 范围 → 断言移速变为 0.7 倍 → 离开恢复
- [ ] `tests/test_module_speed_aura.gd`：创建 2 只敌怪，一只挂 SpeedAura → 另一只进入 10m 范围 → 断言移速变为 1.2 倍
- [ ] `tests/test_module_heal_aura.gd`：创建怪物扣血至 50 → 挂 HealAura → 推进 5s → 断言血量从 50 恢复到 65
