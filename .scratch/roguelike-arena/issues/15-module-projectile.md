...
- [x] 新建 `scripts/modules/module_multishot.gd`，实现：
  ...
  - 非 ATTACK 进入不触发
- [x] `@export` 参数：`burst_count: int = 4`、`burst_interval: float = 0.15`

### ChargedShot 模块（蓄力高伤单发）

- [x] 新建 `scripts/modules/module_charged_shot.gd`，实现：
...
- [x] `@export` 参数：`charge_time: float = 1.2`、`charged_damage_mult: float = 3.0`

### DebuffOnHit 模块（命中减伤）

- [x] 新建 `scripts/modules/module_debuff_on_hit.gd`，实现：
...
- [x] `@export` 参数：`debuff_mult: float = 0.7`、`debuff_duration: float = 2.0`

### 额外：MonsterRanged 辅助接口

- [x] `monster_ranged.gd` 新增 signal：`projectile_hit_player(player)`（在弹体碰撞回调中 emit）
- [x] 此改动使 DebuffOnHit 模块可通过信号解耦，无需修改怪物攻击代码

### 测试

- [x] `tests/test_module_multishot.gd`：挂 MultiShot 模块的远程怪触发 ATTACK → 断言 0.45s 内射出 4 发弹体
- [x] `tests/test_module_charged_shot.gd`：挂 ChargedShot 模块 → 触发 ATTACK → 断言 1.2s 蓄力后才射出 1 发，伤害为 3 倍
- [x] `tests/test_module_debuff_on_hit.gd`：挂 DebuffOnHit 模块的远程怪命中玩家 → 断言 2s 内玩家伤害输出降至 0.7 倍
