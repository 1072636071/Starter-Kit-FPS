...
- [x] 新建 `scripts/modules/module_place_trap_base.gd`，继承 `EnemyModule`，实现：
...
- [x] 新建 `scenes/trap_base.tscn` + `scripts/trap_base.gd`：
  - `extends Area3D`
...
- [x] 新建 `scripts/modules/module_place_trap_poison.gd`，继承 `module_place_trap_base`
- [x] 新建 `scenes/trap_poison.tscn`，继承 `trap_base.gd`：
...
- [x] 新建 `scripts/modules/module_place_trap_damage.gd`，继承 `module_place_trap_base`（覆盖 `trap_scene` 为 `trap_damage.tscn`，`place_cooldown = 6.0`）
- [x] 新建 `scenes/trap_damage.tscn`，继承 `trap_base.gd`：
  - `activate(player)` → 立刻爆炸 AOE：`explosion_radius = 2.5m`，`explosion_damage = 40` → `queue_free()`

### 测试

- [x] `tests/test_module_place_trap.gd`：
...
- [x] `tests/test_trap_poison.gd`：
  - 放置毒陷阱，玩家踩入 → 断言 10s 内扣血 20 ticks × 8 = 160
- [x] `tests/test_trap_damage.gd`：
