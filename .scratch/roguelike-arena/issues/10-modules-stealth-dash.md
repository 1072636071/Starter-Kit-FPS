...
- [ ] 新建 `scripts/modules/module_stealth.gd`，继承 `EnemyModule`，实现：
  ...
  - 非 ATTACK 状态进入不触发
- [ ] `@export` 参数：`stealth_duration: float = 0.5`、`stealth_alpha: float = 0.4`
- [ ] 模块挂为任意 `monster_base` 子类 `.tscn` 的子节点即可工作——不读不写敌人特定字段

### Dash 模块

- [ ] 新建 `scripts/modules/module_dash.gd`，继承 `EnemyModule`，实现：
  ...
  - 非 ATTACK 状态进入不触发
- [ ] `@export` 参数：`dash_distance: float = 5.0`、`dash_duration: float = 0.15`

### 测试

- [ ] `tests/test_module_stealth.gd`：
  - 创建临时 `monster_melee` 实例，挂 Stealth 模块
  - 触发 ATTACK → 断言 0.5s 内 mesh `transparency == 0.4`，0.6s 后恢复 0
- [ ] `tests/test_module_dash.gd`：
