...
- [x] 新建 `scripts/modules/module_berserk_on_damage.gd`，实现：
...
- [x] `@export` 参数：`berserk_damage_mult: float = 1.5`、`berserk_speed_mult: float = 1.3`、`berserk_duration: float = 3.0`、`berserk_cooldown: float = 8.0`

### Shield 模块

- [x] 新建 `scripts/modules/module_shield.gd`，实现：
...
- [x] `@export` 参数：`shield_max: int = 60`

### SelfDestruct 模块

- [x] 新建 `scripts/modules/module_self_destruct.gd`，实现：
...
- [x] `@export` 参数：`detonate_range: float = 3.0`、`fuse_time: float = 0.8`、`explosion_radius: float = 5.0`、`explosion_damage: int = 60`

### ExplodeOnDeath 模块

- [x] 新建 `scripts/modules/module_explode_on_death.gd`，实现：
...
- [x] `@export` 参数：`explosion_radius: float = 4.0`、`explosion_damage: int = 40`

### 测试

- [x] `tests/test_module_berserk.gd`：触发伤害 → 断言 `move_speed` 和 `damage_multiplier` 变为乘后值，3s 后恢复
- [x] `tests/test_module_shield.gd`：攻击有盾敌人 → 断言先扣盾再扣血；盾破后 `shield_broken` 信号发出
- [x] `tests/test_module_self_destruct.gd`：玩家接近 → 断言 0.8s 前摇后爆炸 AOE 伤害 60，敌人死亡
- [x] `tests/test_module_explode_on_death.gd`：杀死敌人 → 断言死亡时爆炸；挂 SelfDestruct 同时挂 ExplodeOnDeath → 断言只爆一次
