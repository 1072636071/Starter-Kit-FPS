...
func _process(delta):
	...
	action_export_json() # Export JSON for FPS project
...
# Export map layout as JSON for the FPS project conversion pipeline.
# Output format: {"cells": [{"x": int, "y": int, "orientation": int, "structure": int}, ...]}
# Copy the generated file to FPS project's resources/city-map-data.json.

func action_export_json():
	if Input.is_action_just_pressed("export_json"):
		print("Exporting map to JSON...")
		...
		for cell in gridmap.get_used_cells():
			cells_data.append({
				...
				"structure": gridmap.get_cell_item(cell),
			})
		
		var json_data := {"cells": cells_data}
		var json_string := JSON.stringify(json_data, "\t")
		
		var file := FileAccess.open("res://map-data.json", FileAccess.WRITE)
		if not file:
			push_error("Failed to write res://map-data.json")
			return
		file.store_string(json_string)
		file.close()
		
		print("Exported %d cells to res://map-data.json" % cells_data.size())
		print("Copy this file to FPS project: resources/city-map-data.json")
